...
if ( ! class_exists( 'Ai_Robot_Job' ) ) :

    abstract class Ai_Robot_Job {
            ...
            if($settings['robot_save_image'] == 'on'){
			    ...
			    foreach ( $srcs as $image_url ) {
                    ...
                    if ($imghost != $current_host) {
                        $file_link = ai_robot_upload_image($image_url, $filename);
        ...
        */
        public function fetch_stream( $url, $headers = array()) {
          ...
          if ( $request['response']['code'] != 200 ) {
              ...
              $this->log[] = array(
                'message'   => '<a href="'.admin_url( 'admin.php?page=ai-robot-integrations' ).'">Note: Please Remember to Setup Youtube API before run this campaign<a>',
                'level'     => 'error'
              );
              return $empty_response;
          }
