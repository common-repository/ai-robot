...
if ( ! class_exists( 'Ai_Robot_Base_Form_Model' ) ) :

	abstract class Ai_Robot_Base_Form_Model {

		const META_KEY = 'ai_robot_form';
		...
		/**
		 * @since 1.0.0
		 * @return Ai_Robot_Form_Field_Model[]
		 */
		 ...
		 * @param $slug
		 *
		 * @return Ai_Robot_Form_Field|null
		 */
		...
		public function get_all_paged( $current_page = 1, $per_page = null, $status = '' ) {
			if ( is_null( $per_page ) ) {
				$per_page = ai_robot_form_view_per_page();
			}
			$args = array(
				'post_type'      => $this->post_type,
				'post_status'    => 'any',
     ...
     */
						...
						if ( 'post' === $map['type'] ) {
							...
							if ( ! empty( $meta['fields'] ) && 'fields' === $map['field'] ) {
								foreach ( $meta['fields'] as $field_data ) {
									$field          = new Ai_Robot_Form_Field_Model( $form_settings );
		...
		public function get_fields_grouped() {
			...
			foreach ( $this->fields as $field ) {
				/** @var Ai_Robot_Form_Field_Model $field */
