(function($){
    ...
    var AiRobotWelcome = {
        ...
        /**
         * Binds events for the Ai Robot Welcome.
         *
         * @since 1.0.0
         * @access private
         * @method _bind
        ...
        _bind: function()
        {
            $( document ).on('click', '.robot-welcome-next', AiRobotWelcome._nextSlider );
            $( document ).on('click', '.robot-welcome-prev', AiRobotWelcome._prevSlider );
            $( document ).on('keyup', '#robot-search', AiRobotWelcome._searchSuggest );
            $( document ).on('click', '.robot-keyword-selected', AiRobotWelcome._selectKeyword );
            $( document ).on('click', '.robot-generate-campaign', AiRobotWelcome._generateCampaign );
            $( document ).on('click', '.robot-slide-trigger-button', AiRobotWelcome._triggerCampaign );
            $( document ).ready( AiRobotWelcome._languageSelector() );
            $( document ).ready( AiRobotWelcome._typeSelector() );
            $( document ).ready( AiRobotWelcome._modeSelector() );
            $( document ).ready( AiRobotWelcome._categorySelector() );
            $( document ).ready( AiRobotWelcome._changeBgColor() );

            // Delay Selector
            $( document ).ready( AiRobotWelcome._rangeSlider() );
        },
         ...
         */
         _triggerCampaign: function( ) {
            ...
            console.log(type);
            var target_url = Ai_Robot_Data.wizard_url + '&source='+ type;
        ...
        _generateCampaign: function( event ) {
            ...
            $.ajax({
                    ...
                    dataType: 'json',
                    data : {
                        action       : 'ai_robot_generate_campaign',
                        fields_data  : fields,
                ...
                .done(function ( options ) {
                    ...
                    if( false === options.success ) {
                        ...
                        var campaigns_url = Ai_Robot_Data.campaigns_url;
        ...
        _nextSlider: function( event ) {
           ...
           if ( currentSlider.hasClass('robot-slide-campaign-name') && $( "input[name='robot_campaign_name']" ).val().length == 0 ){
                ...
                currentSlider.hasClass('robot-slide-temperature') && $( "input[name='temperature']" ).val().length > 2){
                $('.robot-error-message-temperature').css("display", "block");
                return;
            }else{
                AiRobotWelcome._sliderLayerAnimate(direction, svgCoverLayer, pathArray, svgPath);
            }
        ...
        _sliderLayerAnimate: function(direction, svgCoverLayer, paths, svgPath) {
            ...
            var firstCustomMinaAnimation = AiRobotWelcome._bezier(.42, .03, .77, .63, epsilon);
            var secondCustomMinaAnimation = AiRobotWelcome._bezier(.27, .5, .6, .99, epsilon);
        ...
        _addTag: function( textValue ) {
            ...
            remove.addEventListener('click', AiRobotWelcome._deleteTag);
