(function($){
    ...
    var AiRobotAction = {
        ...
        {
            // Document ready.
            $( document ).ready( AiRobotAction._stickyHeader() );
            $( document ).ready( AiRobotAction._loadPopup() );
            $( document ).ready( AiRobotAction._mobileSelect() );
            $( document ).ready( AiRobotAction._rangeSlider() );
            $( document ).ready( AiRobotAction._mainSelect() );
            $( document ).ready( AiRobotAction._postSelect() );
            $( document ).ready( AiRobotAction._typeSelect() );
            $( document ).ready( AiRobotAction._authorSelect() );
            $( document ).ready( AiRobotAction._categorySelect() );
            $( document ).ready( AiRobotAction._sourceSelect() );
            $( document ).ready( AiRobotAction._translatorAPISelector() );
        ...
        /**
         * Binds events for the Ai Robot Action.
         *
         * @since 1.0.0
         * @access private
         * @method _bind
        ...
        _bind: function()
        {
            $( document ).on('click', '.robot-vertical-tab a', AiRobotAction._switchTabs );
            $( document ).on('keyup', '#robot-search', AiRobotAction._searchSuggest );
            $( document ).on('click', '.robot-keyword-selected', AiRobotAction._selectKeyword );
            $( document ).on('click', '#robot-campaign-save', AiRobotAction._saveCampaign );
            $( document ).on('click', '#robot-campaign-publish', AiRobotAction._publishCampaign );
            $( document ).on('click', '.robot-run-campaign-button', AiRobotAction._runCampaign );
            $( document ).on('click', '.sui-tab-item', AiRobotAction._switchSuiTabs );
            $( document ).on('click', '.source-select-list-container', AiRobotAction._sourceSelectTab );
            $( document ).on('click', '.button_add_field', AiRobotAction._appendTemplate );
            $( document ).on('click', '#robot-clear-logs', AiRobotAction._clearLogs );
            $( document ).on('click', '.nav-tab', AiRobotAction._switchWelcomeTabs );
            $( document ).ready( AiRobotAction._languageLocationSelector() );
            ...
            $.ajax({
                ...
                .done(function ( options ) {
                    if( false === options.success ) {
                        ...
                        var campaign_url = Ai_Robot_Data.wizard_url+ '&id=' + options.data + '&source=' + fields['robot_selected_source'];
        ...
        _publishCampaign: function( ) {
            ...
            console.log(Ai_Robot_Data.ajaxurl);
        ...
        _clearLogs: function( event ) {
            ...
            .done(function ( options ) {
                if( false === options.success ) {
                    ...
                    AiRobotAction._displayNoticeMessage(options.data);
